...
subj_detector_path = os.path.join(models_dir, "count_bernoulli_subj_det_model.joblib")
subj_vectorizer_path = os.path.join(models_dir, "count_bernoulli_subj_det_vectorizer.joblib")
...
class TwoStageClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, first_stage_vectorizer_path=first_stage_vectorizer_path, second_stage_vectorizer_path=second_stage_vectorizer_path,
                    neg_cls_min_confidence=0.6, pos_cls_min_confidence=0.6, dim_red=True,
                    use_subjectivity=False, subj_detector_path=subj_detector_path, subj_vectorizer_path=subj_vectorizer_path):
    ...
    def subjectivity_features(self, tokenized_corpus):
        ...
        for i, doc in enumerate(tokenized_corpus):
            ...
            feats.append(1 if np.count_nonzero(np.array(y_pred)) >= len(y_pred) else 0)
    ...
    def predict(self, X: list[list[str]]) -> np.ndarray:
        ...
        for i, cls_proba in enumerate(C_first_stage):
            if self.reject_option(cls_proba) and self.second_stage_vectorizer is not None:
                ...
                rejected_samples_los.append(rejected_sample_los) # i-th document in X
        ...
        if len(rejected_samples_los) > 0:
            rejected_vectors = self.second_stage_vectorizer.transform(rejected_samples_los)
            ...
            if self.use_subjectivity:
                subj_features = self.subjectivity_features(rejected_samples_lol)
                rejected_vectors = hconcat(rejected_vectors, np.expand_dims(subj_features, axis=-1))
        ...
        logger.info(f"SVC was called {len(rejected_samples_los)/len(X)*100:.1f}% of the times.")
